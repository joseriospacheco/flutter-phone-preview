...
function proxyRestRequest(req, res, requestUrl, previewOrigin, lang = 'es') {
    ...
    if (req.headers.host !== new URL(previewOrigin).host ||
        ...
        fail(403, (0, i18n_1.t)(lang, 'rest.forbidden'));
        return;
    }
    let url;
    try {
        ...
        fail(400, (0, i18n_1.t)(lang, 'rest.invalidUrl'));
        return;
    }
    ...
    const timer = setTimeout(() => {
        fail(504, (0, i18n_1.t)(lang, 'rest.timeout'));
    ...
    req.once('error', () => { fail(502, (0, i18n_1.t)(lang, 'rest.interrupted')); cleanup(); });
    const send = (target, method, outgoing, redirects, body) => {
        if (closed || res.writableEnded)
            return;
        if (target.origin === previewOrigin) {
            fail(400, (0, i18n_1.t)(lang, 'rest.redirectSelf'));
            return;
        }
        if (body !== undefined) {
            outgoing = { ...outgoing };
        ...
        try {
            current = transport.request(target, { method, headers: outgoing }, response => {
                ...
                response.on('error', () => { if (activeResponse === response && activeRequest === current)
                    fail(502, (0, i18n_1.t)(lang, 'rest.responseInterrupted')); });
                const status = response.statusCode || 502;
                const location = response.headers.location;
                if ([301, 302, 303, 307, 308].includes(status) && location && redirectMode === 'follow') {
                    if (redirects >= 10) {
                        response.resume();
                        fail(502, (0, i18n_1.t)(lang, 'rest.tooManyRedirects'));
                        return;
                    }
                    let next;
                    try {
                        ...
                        response.resume();
                        fail(502, (0, i18n_1.t)(lang, 'rest.badRedirect'));
                        return;
                    }
                    ...
                    const follow = () => {
                        if (!dropBody && body === undefined && !replayable) {
                            fail(502, (0, i18n_1.t)(lang, 'rest.bodyTooLarge'));
                            return;
                        }
            ...
            fail(502, (0, i18n_1.t)(lang, 'rest.prepareFail'));
            return;
        }
        ...
        current.once('error', () => {
            if (activeRequest === current)
                fail(502, (0, i18n_1.t)(lang, 'rest.connectFail'));
        });
        if (body !== undefined)
            current.end(body);
        else
