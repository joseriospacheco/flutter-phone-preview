...
export function proxyRestRequest(req: http.IncomingMessage, res: http.ServerResponse, requestUrl: URL, previewOrigin: string): void {
  ...
  const send = (target: URL, method: string, outgoing: http.OutgoingHttpHeaders, redirects: number, body?: Buffer) => {
    ...
    if (target.origin === previewOrigin) { fail(400, 'La API no puede redirigir al proxy de vista previa.'); return; }
    if (body !== undefined) {
      outgoing = { ...outgoing };
    ...
    try {
      current = transport.request(target, { method, headers: outgoing }, response => {
        ...
        response.on('error', () => { if (activeResponse === response && activeRequest === current) fail(502, 'La respuesta de la API se interrumpió.'); });
        const status = response.statusCode || 502;
        const location = response.headers.location;
        if ([301, 302, 303, 307, 308].includes(status) && location && redirectMode === 'follow') {
          if (redirects >= 10) { response.resume(); fail(502, 'La API excedió el límite de redirecciones.'); return; }
          let next: URL;
          try { next = destination(location, target); }
          catch { response.resume(); fail(502, 'La API devolvió una redirección inválida.'); return; }
          ...
          const follow = () => {
            if (!dropBody && body === undefined && !replayable) { fail(502, 'No se puede reenviar una carga mayor a 8 MiB tras una redirección.'); return; }
    ...
    current.once('error', () => {
      if (activeRequest === current) fail(502, 'No se pudo conectar con la API. Verifica la URL, la red y el certificado HTTPS.');
    });
