...
function destination(value: string, base?: URL): URL {
...
export function proxyRestRequest(req: http.IncomingMessage, res: http.ServerResponse, requestUrl: URL, previewOrigin: string, lang: Lang = 'es'): void {
  ...
  if (req.headers.host !== new URL(previewOrigin).host ||
    ...
    fail(403, t(lang, 'rest.forbidden'));
    return;
  }
  ...
  try {
    ...
    if (!value) throw new Error('Falta la URL de la API.');
    url = destination(value);
    if (url.origin === previewOrigin) throw new Error('La API no puede apuntar al proxy de vista previa.');
  } catch {
    fail(400, t(lang, 'rest.invalidUrl'));
    return;
  }
  ...
  const pendingRequests = new Set<http.ClientRequest>();
  const pendingResponses = new Set<http.IncomingMessage>();
  let activeRequest: http.ClientRequest | undefined;
  let activeResponse: http.IncomingMessage | undefined;
  let closed = false;
  const timer = setTimeout(() => {
    fail(504, t(lang, 'rest.timeout'));
  ...
  req.on('data', (chunk: Buffer) => {
    ...
    if (bodySize <= 8 * 1024 * 1024 && replayable) chunks.push(Buffer.from(chunk));
    else { replayable = false; chunks.length = 0; }
  });
  req.once('end', () => { bodyEnded = true; });
  req.once('error', () => { fail(502, t(lang, 'rest.interrupted')); cleanup(); });

  const send = (target: URL, method: string, outgoing: http.OutgoingHttpHeaders, redirects: number, body?: Buffer) => {
    if (closed || res.writableEnded) return;
    if (target.origin === previewOrigin) { fail(400, t(lang, 'rest.redirectSelf')); return; }
    if (body !== undefined) {
      outgoing = { ...outgoing };
      delete outgoing['transfer-encoding'];
      if (body.length || outgoing['content-length'] !== undefined) outgoing['content-length'] = body.length;
    }
    const transport = target.protocol === 'https:' ? https : http;
    let current: http.ClientRequest;
    try {
      current = transport.request(target, { method, headers: outgoing }, response => {
        ...
        response.on('error', () => { if (activeResponse === response && activeRequest === current) fail(502, t(lang, 'rest.responseInterrupted')); });
        const status = response.statusCode || 502;
        const location = response.headers.location;
        if ([301, 302, 303, 307, 308].includes(status) && location && redirectMode === 'follow') {
          if (redirects >= 10) { response.resume(); fail(502, t(lang, 'rest.tooManyRedirects')); return; }
          let next: URL;
          try { next = destination(location, target); }
          catch { response.resume(); fail(502, t(lang, 'rest.badRedirect')); return; }
          ...
          if (dropBody) {
            for (const name of ['content-length', 'content-type', 'content-encoding', 'content-language', 'content-location']) delete nextHeaders[name];
          }
          response.resume();
          // A server may redirect before the upload finishes. Wait for its
          // original body before replaying a 307/308 request.
          const follow = () => {
            if (!dropBody && body === undefined && !replayable) { fail(502, t(lang, 'rest.bodyTooLarge')); return; }
            send(next, dropBody ? 'GET' : method, nextHeaders, redirects + 1,
              dropBody ? Buffer.alloc(0) : (body ?? Buffer.concat(chunks)));
          };
          if (bodyEnded) follow(); else req.once('end', follow);
          return;
        }
        ...
        for (const name of Object.keys(responseHeaders)) {
          if (name.startsWith('access-control-') || ['set-cookie', 'set-cookie2', 'clear-site-data',
            'content-security-policy', 'content-security-policy-report-only', 'strict-transport-security'].includes(name)) delete responseHeaders[name];
        }
      ...
      fail(502, t(lang, 'rest.prepareFail'));
      return;
    }
    ...
    current.once('error', () => {
      if (activeRequest === current) fail(502, t(lang, 'rest.connectFail'));
    });
    if (body !== undefined) current.end(body);
