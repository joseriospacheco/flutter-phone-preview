...
export const PREFS_MAX_VALUE_BYTES = 100 * 1024;

export function getPrefsBridge(seedJson: string, endpointPath: string): string {
  return String.raw`(() => {
    const endpointPath = ${JSON.stringify(endpointPath)};
    const maxValueBytes = ${PREFS_MAX_VALUE_BYTES};
    let seed = {};
    try {
